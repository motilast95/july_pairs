...
def generate_rolling_signals(
    ...
    FIXED: Uses only past data to avoid look-ahead bias.
    """
    # FIX: Use shift(1) to avoid look-ahead bias - only use past data
    rolling_mean = spread.shift(1).rolling(window=rolling_window, min_periods=rolling_window).mean()
    rolling_std = spread.shift(1).rolling(window=rolling_window, min_periods=rolling_window).std()
